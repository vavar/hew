  ...
  <md-dialog md-open-from="#fab" md-close-to="#fab" ref="menuModal">
    ...
    <md-dialog-content class="add-order-dialog">
      <form v-on:submit.prevent="addMenu">
        <md-input-container>
          <label>Menu</label>
          <md-select placeholder="Name" name="name" v-model="newMenu.name">
            <md-subheader>Select Menu</md-subheader>
            <md-option v-for="(value, rowIndex) in restaurant.menus" :key="rowIndex" :value="value.name">{{ value.name }} ( {{ value.price }} baht )</md-option>
          </md-select>
        </md-input-container>
        <md-input-container>
