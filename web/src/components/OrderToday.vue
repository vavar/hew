        ...
        <form>
          <md-input-container>
            ...
            <md-select
              ...
              :name="'name'"
              v-model="menu.name">
              <md-option v-for="(value, rowIndex) in menu" :key="rowIndex" value={value.name}>{{ value.name }}</md-option>
            </md-select>
          </md-input-container>
          <md-input-container>
      ...
      <md-table md-sort="restaurant" md-sort-type="desc" @sort="onSort">
        ...
        <md-table-body>
          <md-table-row v-for="(row, rowIndex) in menu" :key="rowIndex" :md-item="row" md-auto-select>
            <md-table-cell v-for="(column, columnIndex) in row" :key="columnIndex" :md-numeric="columnIndex !== 'name'">
              {{ column }}
            </md-table-cell>
            <md-table-cell>
